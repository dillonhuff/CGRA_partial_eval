...
int main(int argc, char** argv) {
  ...
  cout << "top->config_sb 64 = " << bitset<64>(top->config_sb_res) << endl;
  ...
  for (int i = 0; i < strings.size(); i++) {
    ...
    unsigned int tile_id = configAddr & ((1 << 16) - 1);
    unsigned int config_switch = (configAddr >> 16) & ((1 << 8) - 1);

    top->config_addr = configAddr; // Insert config
    top->config_data = configData; // Insert data
    top->config_en = (tile_id == 0x15) && (config_switch == 8);
    cout << "\tconfig enable = " << (int) top->config_en << endl;
    top->clk = 0;
    top->eval();
